...
// --- COMPONENT SEARCHABLE SELECT (Đã nâng cấp UI) ---
const SearchableSelect = ({ options, value, onChange, placeholder, labelKey = 'name' }: any) => {
  ...
  return (
    <div className="relative w-full" ref={wrapperRef}>
      ...
      {isOpen && (
        // min-w-[300px]: Đảm bảo menu luôn đủ rộng để đọc chữ
        <div className="absolute z-50 mt-1 bg-white border border-gray-200 rounded-lg shadow-2xl max-h-60 overflow-hidden flex flex-col min-w-[300px] left-0">
...
export default function DealsPage() {
  ...
  // State Danh sách Item: Thêm trường `is_custom`
  // Nếu is_custom=true -> Cho nhập tên (name) thoải mái, product_id = null
  ...
  const loadData = async () => {
    const d = await supabase.from('deals').select('*, customers(name), profiles(full_name)').order('created_at', { ascending: false })
  ...
  // Xử lý khi chọn SP từ danh sách
  ...
  // Xử lý thay đổi số lượng, giá, hoặc tên (cho custom item)
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Validate: Dòng custom phải có tên
    ...
    try {
      ...
      // B2: Lưu Deal Items (Lưu cả item_name vào DB)
      const itemsPayload = selectedItems.map(item => ({
        deal_id: newDeal.id,
        product_id: item.is_custom ? null : item.product_id, // Nếu custom thì null
        item_name: item.name, // Lưu tên (quan trọng cho custom item)
  ...
  const resetForm = () => { setFormData({ title: '', customer_id: '', stage: 'NEW', expected_close_date: '' }); setSelectedItems([]) }
         ...
         {filteredDeals.map((deal) => (
           <div key={deal.id} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition flex flex-col justify-between border-l-4 border-l-transparent hover:border-l-yellow-500 group">
             <div>
                ...
                <h3 className="font-bold text-gray-900 truncate text-base mb-1">{deal.title}</h3>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-4 z-50 animate-in fade-in backdrop-blur-sm">
          <div className="bg-white rounded-xl w-full max-w-5xl shadow-2xl flex flex-col max-h-[95vh]">
             ...
             <div className="px-8 py-5 border-b border-gray-100 flex justify-between items-center bg-gray-50 rounded-t-xl">
               <h2 className="text-xl font-bold text-gray-900 flex items-center gap-2"><Briefcase className="h-5 w-5 text-red-600"/> {formData.title ? 'Cập nhật Deal' : 'Tạo Cơ hội mới'}</h2>
             ...
             <div className="p-8 overflow-y-auto flex-1 bg-white">
                <form id="dealForm" onSubmit={handleSave} className="space-y-8">
                  ...
                  {/* Cột 2: Bảng Sản phẩm & Dịch vụ (LỚN HƠN) */}
                  <div className="border-t border-gray-100 pt-6">
                    ...
                    <div className="bg-gray-50 rounded-xl border border-gray-200 overflow-hidden shadow-sm">
                       {/* Header bảng */}
                       <div className="grid grid-cols-12 gap-4 p-3 bg-gray-100 text-xs font-bold text-gray-500 uppercase border-b border-gray-200">
                       ...
                       <div className="p-3 space-y-3">
                          ...
                          {selectedItems.map((item, index) => (
                             <div key={index} className="grid grid-cols-12 gap-4 items-center bg-white p-2 rounded border border-gray-100 shadow-sm">
                                {/* Cột Tên */}
                                ...
                                <div className="col-span-2">
                                   <input type="number" className={`w-full p-2.5 border border-gray-300 rounded text-right text-sm outline-none focus:border-red-500 ${item.is_custom ? 'bg-yellow-50' : ''}`}
             ...
             <div className="px-8 py-5 border-t border-gray-100 bg-gray-50 flex justify-end gap-4 rounded-b-xl">
               ...
               <button type="submit" form="dealForm" disabled={submitting} className="px-8 py-2.5 bg-red-600 text-white rounded-lg text-sm font-bold hover:bg-red-700 shadow-lg shadow-red-200 flex items-center gap-2 transition transform active:scale-95">
                 {submitting ? <Loader2 className="h-4 w-4 animate-spin"/> : 'Lưu Cơ hội'}
               </button>
             </div>
          </div>
        </div>
